    ...
    if (req.method === 'POST') {
        try {
            ...
            if(bookmark.url){
                ...
                const existingBookmark = await bmCollection.findOne({ url: bookmark.url }, { projection: { _id: 1, user: 1, referencedBy: 1 } });
                let isOwner = true;
                if(existingBookmark && existingBookmark.user!== userEmail ){
                    isOwner = false;
                }
                const rootDomain = extractRootDomain(bookmark.url);
                bookmark.source = rootDomain;
