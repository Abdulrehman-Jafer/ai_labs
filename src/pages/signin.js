...
import { app } from '../firebase/client'; // Ensure this is the client-side config
...
export default function Login() {
    ...
    event.preventDefault();
        try {
            ...
            if (response.ok) {
                // Server has successfully created the session, redirect to dashboard
                router.push('/dashboard');
            } else {
                setErrorMessage(message || 'Failed to sign in');
            }
        } catch (error) {
    ...
    const handleGoogleSignIn = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setErrorMessage(message || 'Failed to signin with Google login');
            }
        } catch (error) {
        ...
        <div className="bg-gray-100 flex rounded-2xl shadow-lg max-w-3xl p-5 items-center">
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-4">
                ...
                <div className="mt-3 text-xs flex justify-between items-center text-[#002D74]">
                    ...
                    <button onClick={handleRegisterClick} className="py-2 px-5 bg-white border rounded-xl hover:scale-110 duration-300">Register</button>
                </div>
            </div>

            <div className="md:block hidden w-1/2">
                <img className="rounded-2xl" src="https://images.unsplash.com/photo-1570323085210-cc920ab3f50f?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=387&q=80" />
            </div>
